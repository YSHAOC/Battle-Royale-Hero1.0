...
namespace JKFrame
{
    public static class JKEventListenerExtend
    {
        ...
        //每个扩展的组件 都会在 （该GameObject中） 自动添加 -- （JKEventListener脚本组件）
        ...
        {
            JKEventListener lis = GetOrAddJKEventListener(com);//添加（JKEventListener脚本组件） 
            lis.AddListener(eventType, action, args);  //在（JKEventListener脚本组件）里面添加事件
        }
        //取消移除--拓展事件
        ...
        {
            JKEventListener lis = GetOrAddJKEventListener(com); //获取该GameObject中添加的（JKEventListener脚本组件） 
            lis.RemoveListener(eventType, action, checkArgs, args); //在（JKEventListener脚本组件）里面取消事件
        }
        ...
        {
            //添加（JKEventListener脚本组件）并在 （JKEventListener脚本组件）里面添加事件
            //在上面脚本中字典 eventInfoDic{  ”JKEventType.OnMouseEnter“  =  action ++ }
        ...
        {
            RemoveEventListener(com, JKEventType.OnClick, action, checkArgs, args); //获取该GameObject中添加的（JKEventListener脚本组件）并在 （JKEventListener脚本组件）里面取消事件
        }
