...
public abstract class Monster_StateBase : StateBase
{
    ...
    {
        if (Vector3.Distance(monster.transform.position, player.transform.position) < 9.0f)
        {
    ...
    /// 最小追击范围
    /// </summary>
    protected bool CheckStopFollowAndChangeState()
    {
        if (Vector3.Distance(monster.transform.position, player.transform.position) > 6.0f)
        {
